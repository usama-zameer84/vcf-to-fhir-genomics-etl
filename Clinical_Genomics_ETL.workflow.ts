...
`,
    ...
    @node({
        id: 'bac3286f-570c-4a43-85be-a8d4312aa064',
    ...
    LogSuccessToDatabase = {
        ...
        query: `INSERT INTO vcf_processing_log
  (sample_id, patient_id, lab_id, genome_build, variant_count,
   pass_count, pathogenic_count, fhir_report_id, status, processed_at)
VALUES (
  '{{ $('Build FHIR Diagnostic Report').item.json.sample_id }}',
  '{{ $('Build FHIR Diagnostic Report').item.json.patient_id }}',
  '{{ $('Build FHIR Diagnostic Report').item.json.lab_id }}',
  '{{ $('Build FHIR Diagnostic Report').item.json.genome_build }}',
  {{ $('Build FHIR Diagnostic Report').item.json.observation_count }},
  {{ $('Build FHIR Diagnostic Report').item.json.pass_count }},
  {{ $('Build FHIR Diagnostic Report').item.json.pathogenic_count }},
  'report-{{ $('Build FHIR Diagnostic Report').item.json.sample_id }}',
  'success',
  NOW()
)
ON CONFLICT (sample_id) DO UPDATE
  SET status           = 'success',
    ...
    @node({
        id: 'e48d439d-e143-4e20-9bfa-2dbb3ffbbadb',
        webhookId: '5da43478-8a39-4aa2-83a3-17038c913d4c',
        name: 'Notify Slack',
    ...
    NotifySlack = {
        ...
        channelId: {
            __rl: true,
            value: '#n8n-notifications',
            mode: 'name',
        },
        text: `=VCF to FHIR ETL Complete
• Sample: \`{{ $('Build FHIR Diagnostic Report').item.json.sample_id }}\`
• Patient: \`{{ $('Build FHIR Diagnostic Report').item.json.patient_id }}\`
• Lab: \`{{ $('Build FHIR Diagnostic Report').item.json.lab_id }}\`
• Variants processed: {{ $('Build FHIR Diagnostic Report').item.json.observation_count }}
• PASS variants: {{ $('Build FHIR Diagnostic Report').item.json.pass_count }}
• Pathogenic / LP: {{ $('Build FHIR Diagnostic Report').item.json.pathogenic_count }}
• Genome build: {{ $('Build FHIR Diagnostic Report').item.json.genome_build }}
• FHIR Report: \`DiagnosticReport/report-{{ $('Build FHIR Diagnostic Report').item.json.sample_id }}\`
• View Report: <{{ $('Extract Report ID').item.json.fhir_html_url }}|Open in HAPI FHIR>
`,
        otherOptions: {},
    };
...
return [{ json: {
  ...
  timestamp:     new Date().toISOString()
    ...
    @node({
        id: 'b5b9216a-461f-443e-a10c-211f1f4d6917',
    ...
    LogErrorToDatabase = {
        ...
        query: `INSERT INTO vcf_processing_log
  (sample_id, patient_id, lab_id, genome_build, variant_count,
   pass_count, pathogenic_count, fhir_report_id, status, error_message, processed_at)
VALUES ('UNKNOWN', 'UNKNOWN', 'UNKNOWN', 'UNKNOWN', 0, 0, 0, 'N/A',
  'failed', '{{ $json.error_message.replace(/'/g, "''") }}', NOW());`,
        options: {},
    };

    @node({
        id: 'd7a31a1e-a635-477b-97b7-61a8fef1732f',
    ...
    ArchitectureNotes = {
        content: `## PostgreSQL Query for logging
CREATE TABLE IF NOT EXISTS vcf_processing_log (
  id               BIGSERIAL PRIMARY KEY,
  sample_id        TEXT        NOT NULL,
  patient_id       TEXT        NOT NULL DEFAULT 'UNKNOWN',
  lab_id           TEXT        NOT NULL DEFAULT 'UNKNOWN',
  genome_build     TEXT,
...
CREATE INDEX IF NOT EXISTS idx_vcf_log_status    ON vcf_processing_log (status);
CREATE INDEX IF NOT EXISTS idx_vcf_log_patient   ON vcf_processing_log (patient_id);
CREATE INDEX IF NOT EXISTS idx_vcf_log_processed ON vcf_processing_log (processed_at DESC);
`,
        height: 464,
        width: 656,
        color: 5,
    ...
    defineRouting() {
        ...
        this.ExtractReportId.out(0).to(this.LogSuccessToDatabase.in(0));
        this.LogSuccessToDatabase.out(0).to(this.NotifySlack.in(0));
        this.NotifySlack.out(0).to(this.FormatHtmlReport.in(0));
        this.FormatHtmlReport.out(0).to(this.ReturnHttpReport.in(0));
        
        this.ErrorTrigger.out(0).to(this.FormatErrorDetails.in(0));
        this.FormatErrorDetails.out(0).to(this.LogErrorToDatabase.in(0));
    }
}